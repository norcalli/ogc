...
static /*TokenID*/ Token GetToken(const char*& str, size_t* length) {
  ...
  *length = 1;
  if (IsFunctionSeparator(*str))
    return kFunctionSeparator;
  if (*str == '(')
...
void ShuntingYard<T>::ProcessOperator(Operator* operation) {
  ...
  while (!operators_.empty()
         && precedence < operators_.top()->precedence) {
    dprintf("ProcessOperatorLoop:\tOperator(%s vs %s);\tPrecedence(%d vs %d)\n",
            operation->name, operators_.top()->name, precedence, operators_.top()->precedence);
    // DEBUGOUT;
    PopOperator();
    // calculator_.HandleOperator(operators_.top());
    // operators_.pop();
...
class State {
 public:
  State() { negate_ = true; impl_mult_ = false; }
...
#ifdef DEBUG
    if (str == start)
      throw "Forgot to move str iterator forward...";
#endif  // DEBUG
  }
